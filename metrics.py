...
def compare_mssim(x_true, x_pred, data_range, multidimension):
    ...
    mssim = [compare_ssim(x_true[:, :, i], x_pred[:, :, i], data_range=data_range, multichannel=False)
             for i in range(x_true.shape[2])]
...
def quality_assessment(x_true, x_pred, data_range, ratio, multi_dimension=False, block_size=8):
    ...
    result = {'MPSNR': compare_mpsnr(x_true, x_pred, data_range),
              'MSSIM': compare_mssim(x_true, x_pred, data_range, multi_dimension),
              'ERGAS': compare_ergas(x_true, x_pred, ratio),
